...
func (r *ClaimRepository) CreateReceiptFile(
	...
	created, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[model.ReceiptFile])
	if err != nil {
...
func (r *ClaimRepository) CreateClaim(ctx context.Context, c *model.Claim) (*model.Claim, error) {
	rows, err := r.db.Query(ctx, `
		INSERT INTO claims (user_id, receipt_file_id, business_purpose, claimed_date, expense_category)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING
			id, user_id, receipt_file_id, business_purpose, claimed_date, expense_category,
	...
	`, c.UserID, c.ReceiptFileID, c.BusinessPurpose, c.ClaimedDate, string(c.ExpenseCategory))
	if err != nil {
		return nil, fmt.Errorf("create claim: %w", err)
	}

	created, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[model.Claim])
	if err != nil {
...
func (r *ClaimRepository) FindReceiptFileByHash(ctx context.Context, hash string) (*model.ReceiptFile, error) {
	...
	rf, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[model.ReceiptFile])
	if err != nil {
...
func (r *ClaimRepository) GetClaimByID(ctx context.Context, id uuid.UUID) (*model.Claim, error) {
	...
	claim, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[model.Claim])
	if err != nil {
		return nil, err // callers check pgx.ErrNoRows
	}
...
func (r *ClaimRepository) GetClaimsByUserID(ctx context.Context, userID string) ([]model.Claim, error) {
	...
	claims, err := pgx.CollectRows(rows, pgx.RowToStructByName[model.Claim])
	if err != nil {
